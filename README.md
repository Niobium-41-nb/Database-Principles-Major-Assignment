...
1. **需求分析文档**
   - [需求分析报告、数据项清单、数据结构清单](docs/在线判题系统（OJ）需求分析报告.md)
   - [数据流图](docs/在线评测系统数据流图（DFD）.md)
   - [数据字典](docs/在线评测系统数据字典（DD）.md)
   - [处理过程说明](docs/处理过程说明.md)

2. **概念结构设计文档**
   - 局部E-R图（分E-R图）
   - [全局E-R图（集成后的基本E-R图）](database/E-R图/E-R图.mermaid)
   - E-R图说明（包括实体、属性、联系的描述）

3. **逻辑结构设计文档**
   - 关系模式（表结构）
   - 关系模式的转换说明（如1:1、1:n、m:n联系的转换方式）
   - 视图（View）设计（如有）
   - 数据模型优化说明（如规范化过程、水平/垂直分解等）

4. **物理设计文档**
   - 存取方法选择（如索引、聚簇、HASH等）
   - 存储结构设计（如数据存放位置、分区策略）
   - 系统配置参数说明（如缓冲区大小、并发用户数等）

5. **数据库实施文件**
   - 建表SQL脚本（包括表、索引、视图、约束等）
   - 示例数据插入脚本（用于试运行）
   - 应用程序关键代码或模块说明（如查询、更新操作）
